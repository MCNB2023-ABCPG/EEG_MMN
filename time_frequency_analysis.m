...
for i = 1:numel(folder_base_sub)
    ...
    % Crop
    wavelet_estimated_power_file = spm_select('FPList', folder_path_stats, '^tf_aefdfMspm.*\.mat$');
    job{1}.spm.meeg.preproc.crop.D = {wavelet_estimated_power_file};
    job{1}.spm.meeg.preproc.crop.timewin = [-100 800];
    job{1}.spm.meeg.preproc.crop.freqwin = [-Inf Inf];
    job{1}.spm.meeg.preproc.crop.channels{1}.all = 'all';
    job{1}.spm.meeg.preproc.crop.prefix = 'p';
    spm_jobman('run', job);


    wavelet_estimated_phase_file = spm_select('FPList', folder_path_stats, '^tph_aefdfMspm.*\.mat$')
    job{2}.spm.meeg.preproc.crop.D = {wavelet_estimated_phase_file};
    job{2}.spm.meeg.preproc.crop.timewin = [-100 800];
